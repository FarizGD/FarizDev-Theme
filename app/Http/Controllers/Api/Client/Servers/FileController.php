...
use Pterodactyl\Services\Admin\AdminFileBlockingService;
...
class FileController extends ClientApiController
{
    ...
    public function __construct(
        ...
        private AdminFileBlockingService $fileBlockingService,
    ) {
        parent::__construct();
    }
    ...
    {
        $maxBytes = config('pterodactyl.files.max_edit_size');
        ...
        if ($this->fileBlockingService->shouldEnforce($request->user())) {
            ...
            $this->fileBlockingService->enforceForWrite($request->user(), $response);
        } else {
            $response = $this->fileRepository->setServer($server)->getContent(
                $request->get('file'),
                $maxBytes
    ...
    {
        if ($this->fileBlockingService->shouldEnforce($request->user())) {
            $this->fileBlockingService->enforceForContent(
                $request->user(),
                $server,
                $request->get('file'),
                $this->fileRepository,
